...
Written in C#.NET (.NET 4.0) using WPF.
...
The IS_ScalePlayer accepts a lambda to set up the channels it uses for playing each note. In this case we add a sine wave oscillator and a terminator (read about terminators below).
...
For basic usage look into the ConfSetup.cs file and modify the Setup method which is responsible with setting up the input source and output channel.